...
class LogoutUseCaseTest {
  ...
  class WhenAuthPortLogoutFailsSilently {
    ...
    void setUp() {
      ...
      doThrow(new RuntimeException("Keycloak unavailable")).when(authPort).logout(refreshToken);
    }
    ...
    void should_PropagateException_When_AuthPortThrows() {
      // When & Then
      org.assertj.core.api.Assertions.assertThatThrownBy(() -> logoutUseCase.execute(refreshToken))
