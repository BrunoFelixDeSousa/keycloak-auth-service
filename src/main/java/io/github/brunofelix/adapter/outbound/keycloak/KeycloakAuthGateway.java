...
public class KeycloakAuthGateway implements AuthPort {
  ...
  @Inject @RestClient private KeycloakTokenClient tokenClient;
  ...
  public TokenResponse login(AuthCredentials credentials) {
    LOGGER.debug(
        "[KeycloakAuthGateway:login] Chamando token endpoint do Keycloak. realm={}", realm);
  ...
  public TokenResponse refreshToken(String refreshToken) throws AuthenticationException {
    LOGGER.debug(
        "[KeycloakAuthGateway:refreshToken] Chamando refresh token no Keycloak. realm={}", realm);
  ...
  public void logout(String refreshToken) throws AuthenticationException {
    LOGGER.debug(
        "[KeycloakAuthGateway:logout] Revogando refresh token no Keycloak. realm={}", realm);
    ...
    try {
      ...
      LOGGER.warn(
          "[KeycloakAuthGateway:logout] Falha ao revogar token. Status: {}",
  ...
  private TokenResponse fetchToken(MultivaluedHashMap<String, String> form, String subject) {
    try {
      ...
      LOGGER.warn(
          "[KeycloakAuthGateway:fetchToken] Falha na autenticao. subject={}, status={}",
          subject,
          status);
